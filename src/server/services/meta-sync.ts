...
export async function syncIgStats(): Promise<SyncResult> {
  ...
  await syncAdStats(token.access_token, localPosts, media.data, result)
...
/**
 * Fetch ad insights from Meta Ads API and merge spend data into matched posts.
 * Matches against both Content Studio posts and standalone IG posts.
...
async function syncAdStats(
  ...
  try {
    ...
    if (!adAccountId) {
      ...
      console.log('[ads] Ad accounts:', accounts.data.map(a => `"${a.name}" (${a.id})`).join(', '))
    ...
    result.ads_found = adsData.data?.length || 0
    console.log(`[ads] Found ${result.ads_found} ads in account ${adAccountId}`)
    ...
    for (const ad of adsData.data) {
      ...
      // Try matching against Content Studio posts first
      const localPost = localPosts.find(p => {
        if (!p.caption) return false
        return p.caption.trim().toLowerCase().startsWith(key)
      })
      ...
      if (localPost) {
        ...
        matched++
        console.log(`[ads] Matched to studio post: "${caption.slice(0, 40)}" - spend: ${spend}`)
        continue
      }

      // Fall back to matching against IG posts from API
      const igPost = igPosts.find(p => {
        if (!p.caption) return false
        return p.caption.trim().toLowerCase().startsWith(key)
      })
      ...
      if (igPost) {
        ...
        matched++
        console.log(`[ads] Matched to IG post: "${caption.slice(0, 40)}" - spend: ${spend}`)
      }
    }

    result.ads_matched = matched
    console.log(`[ads] Matched ${matched} ads to posts`)
  } catch (err) {
    result.ads_error = (err as Error).message
